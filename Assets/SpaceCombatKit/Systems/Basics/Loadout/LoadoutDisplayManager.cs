...
namespace VSX.UniversalVehicleCombat
{
	...
	public class LoadoutDisplayManager : MonoBehaviour 
	{
		...
		[SerializeField]
        protected bool focusCameraOnMount = false;

		[SerializeField]
        protected LoadoutCameraPositionController cameraPositionController;
		...
		[SerializeField]
        protected float cameraRotateSpeed;

        [SerializeField]
        protected float cameraFocusOnMountDistance = 4;
		...
		/// <summary>
        /// Focus the camera on a module mount on a display vehicle in the loadout menu.
        /// </summary>
		void CameraFocus()
		{
			...
			{
				targetPosition = cameraPositionController.transform.position;
				lookPosition = cameraPositionController.transform.position + cameraPositionController.transform.forward * 1000;
			}
			else
			{
				targetPosition = focusedMount.position + (focusedMount.position - focusedVehicleTransform.position).normalized + focusedMount.up;
                targetPosition = focusedMount.position + (targetPosition - focusedMount.position).normalized * cameraFocusOnMountDistance;
			...
			cameraTransform.LookAt(lookPosition);
