...
namespace VSX.UniversalVehicleCombat
{
	...
	public class CapitalShipCombatBehaviour : AISpaceshipBehaviour 
	{
		...
		float GetPrimaryFiringSolutionQuality()
		{
            ...
            if (weapons.GimbaledWeapons.Count > 0)
            {
                float angleToTarget;
                float val = 0;
                foreach (GimbalController gimbaledWeapon in weapons.GimbaledWeapons)
                {
                    
                    gimbaledWeapon.TrackPosition(decisionInfo.targetPosition, out angleToTarget, false);
                ...
                val /= weapons.GimbaledWeapons.Count;
